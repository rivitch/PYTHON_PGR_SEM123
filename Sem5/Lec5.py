...
res1 = {None: item for item in data if item > 4}    # {6:None, 8:None, 10:None, 12:None}   ХЭШ!!! + если наоборот - перезапись ключа
...
# создание генератора не в одну строку, а как отдельную функцию.

# факториал чисел от одного до n.
...
# --------
#Команда yield  (не тратится память)
...
# ------------------
# Функции iter и next для генераторов могут работать с созданными генераторами. Например так:
...
#++++++++++++++++++++
# Домашнее задание
# Возьмите несколько задач из прошлых уроков, где вы создавали функции и
# сделайте из них генераторы. Внесите правки в код, чтобы он работал без ошибок в
# новой реализации.
